...
export class Server {
  ...
  private onJoinRoom(socket: Socket, room: string, name: string) {
    ...
    const round = game ? game.round() : undefined;
  ...
  private onChooseRole(socket: Socket, role: Roles) {
    if (socket.rooms.size === 0 || !socket.data.room) return socket.emit('room error');

    const game = Server.instance.games.get(socket.data.room);
    if (!game) return socket.emit('room error');
    ...
    if (round.setRole(socket, role)) {
      ...
      if (otherSocket && otherSocket.id === socket.id) round[otherRole] = undefined;
        ...
        if (room !== undefined && room.size === 2) {
          const socketId = Array.from(room).find((id) => id !== socket.id);
          if (!socketId) return socket.emit('room error');

          const sndSocket = Server.instance.server.sockets.sockets.get(socketId);
  ...
  private setPin(socket: Socket, prohibitedRole: Roles) {
    if (!socket.data.room || !Server.instance.games.has(socket.data.room)) {
      socket.emit('room error');
    ...
    if ((socket !== game.round().setter && socket !== game.round().guesser) || socket === game.round()[prohibitedRole]) {
      socket.emit('room error');
      new Logger().warn(socket === game.round().setter, socket === game.round().guesser, socket === game.round()[prohibitedRole]);
      return false;
    }
  ...
  private onSetPin(socket: Socket, pin: 1 | 2 | 3 | 4, color: PinColor | undefined) {
    if (!socket.data.room) return socket.emit('room error');
